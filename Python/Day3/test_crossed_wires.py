...
def test_mark_vector():
    grid = Grid(10)
    updated_coord = grid.mark_vector(Vector(Direction.DOWN, 4), grid.origin)
    assert(updated_coord == (0, -4))
    assert(grid.mark_vector(Vector(Direction.RIGHT, 2), updated_coord) == (2, -4))
    assert(grid.get_coord_val(updated_coord) == 1)  # vertex should be 1
    assert(sum(row.count(1) for row in grid.grid) == 6)
...
#     print([CrossedWires.manhattan_distance(cw.grid.origin, intersection) for intersection in cw.get_intersections()])
#
#
# def test_example_3():
...
#     print([intersection for intersection in cw.get_intersections()])
#     assert (min(
#         [CrossedWires.manhattan_distance(cw.grid.origin, intersection) for intersection in cw.get_intersections()]) == 135)
