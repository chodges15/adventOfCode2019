...
object TestProgram {
  ...
  def executeInstruction(op: OpCode, memory: Vector[Int], input: Int = 0): Option[(Vector[Int], Int)] = {
    op match {
      case Terminate() => None
      case Add(left, right, dest) => Some(memory.updated(dest, memory(left) + memory(right)), large_instruction_size)
      case Multiply(left, right, dest) => Some(memory.updated(dest, memory(left) * memory(right)), large_instruction_size)
  ...
  def parseNextInstruction(instruction: Vector[Int]): OpCode = {
    ...
    instruction.head match {
      ...
      case 3 => Input(instruction(LEFT_POS))
      case 4 => Output(instruction(LEFT_POS))
      case 99 => Terminate()
    }
  }
